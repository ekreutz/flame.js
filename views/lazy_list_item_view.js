...
Flame.LazyListViewStates.MouseIsDown = Flame.State.extend({
    ...
    contextMenu: function (event) {
        ...
        var owner = this.get('owner');
        if(owner.contextMenuItems == undefined) {
            return false;
        }
        var cMenu = Flame.MenuView.createWithMixins({
            target: this,
            items: function () {
                return owner.contextMenuItems();
            }.property('Rui.currentUserController.allowBatchRuns')
        });

        cMenu.popup(this, Flame.POSITION_LEFT | Flame.POSITION_BELOW);

        var layout = cMenu.get('layout');
        layout.left += event.offsetX;
...
Flame.LazyListItemView = Flame.ListItemView.extend(Flame.Statechart, {
    ...
    contextMenu: function (event) {
        return this.invokeStateMethod('contextMenu', event);
    },
